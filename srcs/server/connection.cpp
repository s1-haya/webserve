...
Connection::IpList Connection::ResolveHostName(const std::string &hostname) {
	...
	if (status != 0) {
		throw SystemErrorException("getaddrinfo failed: " + std::string(gai_strerror(status)));
	}
...
int Connection::Connect(const HostPortPair &host_port) {
	...
	if (!bind_result.IsOk()) {
		throw SystemErrorException("bind failed: " + std::string(strerror(errno)));
	}
	...
	if (listen(server_fd, 3) == SYSTEM_ERROR) {
		close(server_fd);
		throw SystemErrorException("listen failed: " + std::string(strerror(errno)));
	}
...
Connection::IpPortPair Connection::GetListenIpPort(int client_fd) {
	...
	if (getsockname(client_fd, (struct sockaddr *)&listen_sock_addr, &listen_sock_addr_len) ==
		SYSTEM_ERROR) {
		throw SystemErrorException("getsockname failed: " + std::string(strerror(errno)));
	}
...
ClientInfo Connection::Accept(int server_fd) {
	...
	if (client_fd == SYSTEM_ERROR) {
		throw SystemErrorException("accept failed: " + std::string(strerror(errno)));
	}
