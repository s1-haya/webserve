...
int TestDeleteNoContent2ExistingFileWithBodyMessage(
	...
	http::ClientInfos client_infos         = CreateClientInfos(request::DELETE_204_2_EXISTING_FILE_WITH_BODY_MESSAGE);
	std::string       expected_status_line = EXPECTED_STATUS_LINE_NO_CONTENT;
	std::string       expected_body_message = EXPECTED_BODY_MESSAGE_NO_CONTENT;
	HeaderFields      expected_header_fields;
	expected_header_fields[http::CONNECTION]     = http::CLOSE;
...
int TestDeleteNoContent3ExistingFileThenNotFoundOnSecondAttempt(
	...
	http::ClientInfos client_infos         = CreateClientInfos(request::DELETE_204_3_EXISTING_FILE_THEN_404_ON_SECOND_ATTEMPT);
	...
	expected_header_fields[http::CONTENT_LENGTH] = utils::ToString(expected_body_message_no_content.length());
	expected_header_fields[http::CONTENT_TYPE]   = http::TEXT_HTML;
	expected_header_fields[http::SERVER]         = http::SERVER_VERSION;
	...
	std::string request_buffer = "DELETE /upload/delete_file HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n";
	http::HttpResult expected_no_content = CreateHttpResult(true, true, request_buffer, expected_response_no_content);
	...
	expected_header_fields[http::CONNECTION]     = http::CLOSE;
	expected_header_fields[http::CONTENT_LENGTH] = utils::ToString(expected_body_message_not_found.length());
	...
	http::HttpResult expected_not_found = CreateHttpResult(true, false, "", expected_response_not_found);
