...
struct MethodArgument {
	...
	) : path(path), method(method), allow_methods(allow_methods), request_body_message(request_body_message), response_body_message(response_body_message) {}
	const std::string &path;
	const std::string            &method;
	const AllowMethods &allow_methods;
	const std::string            &request_body_message;
...
int MethodHandlerResult(const MethodArgument& srcs, const std::string &expected_body_message) {
	...
	try {
		http::Method::Handler(
			srcs.path, srcs.method, srcs.allow_methods, srcs.request_body_message, srcs.response_body_message
		);
		result = HandleResult(srcs.response_body_message, expected_body_message);

	} catch (const http::HttpException& e) {
		srcs.response_body_message = http::HttpResponse::CreateDefaultBodyMessageFormat(e.GetStatusCode());
		result = HandleResult(srcs.response_body_message, expected_body_message);
		std::cerr << utils::color::GRAY << e.what() << utils::color::RESET
									 << std::endl;
	}
	return result;
}
...
int main(void) {
	...
	// GET test
	// ファイルが存在する場合
	ret_code |= MethodHandlerResult(MethodArgument("test/file.txt", http::GET, allow_methods, request, response), expected_file);
	...
	// ディレクトリの場合かつ'/'がない場合
	ret_code |= MethodHandlerResult(
		MethodArgument("test/directory", http::GET, allow_methods, request, response), expected_redirect
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument("ok.txt", http::POST, allow_methods, post_test1_request_body_message, response),
		expected_created
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument("ok.txt", http::POST, allow_methods, post_test2_request_body_message, response),
		expected_no_content
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument(
			"test/directory", http::POST, allow_methods, request, response
		),
		expected_forbidden
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument(
			"ok.txt", http::DELETE, allow_methods, request, response
		),
		expected_no_content
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument(
			"not_found.txt", http::DELETE, allow_methods, request, response
		),
		expected_not_found
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument(
			"test", http::DELETE, allow_methods, request, response
		),
		expected_forbidden
	);
	...
	ret_code |= MethodHandlerResult(
		MethodArgument(
			"test/s", http::DELETE, allow_methods, request, response
		),
		expected_forbidden
	);
