...
def wait_for_container(container_name, timeout=60, interval=10):
    ...
    while time.time() - start_time < timeout:
        result = run_command(['make', 'ps'], test_nginx_dir)
...
def test_nginx():
    # イメージをビルドし、コンテナを起動
    run_command(['make', 'build'], test_nginx_dir)
    run_command(['make', 'run'], test_nginx_dir)
    ...
    # コンテナを停止し削除
    result = run_command(['make', 'fclean'], test_nginx_dir)
